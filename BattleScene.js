...
(function(Dnd) 
{
    ...
    {
        var _super = Dnd.BaseScene;
        Laya.class(BattleScene, "Dnd.BattleScene", _super);
        ...
        function BattleScene(props) 
        {
            ...
            (function()
            {
                var vip_open_lv = Dnd.VipData.GetVipItemOpenLevel("value21");
                var vip_open = (Dnd.Player.vip >= vip_open_lv);
                var player_data = Dnd.VipData.GetItemData(1568);
                var player_open_lv = player_data.openLevel;
                var player_open = (Dnd.Player.level >= player_open_lv);
            ...
            var scale = Dnd.Env.gameWidth / (Dnd.Env.gameHeight / 16 * 9);
            if( scale > 1 ) 
                scale = 1;
            ...
            if (SHOW_CAM_POS)
            {
                this.camera.addComponent(Dnd.CameraMoveScript);      
            }      
            ...
            this.battleManager = new Dnd.BattleManager(this);
            ...
            if (Dnd.GuideManager.isGuiding(Dnd.GuideStep.GuideWorld))
            {
                var cryChild = new Dnd.CryChild();
                cryChild.Init(0);
            ...
            if (Dnd.GuideManager.isGuiding(Dnd.GuideStep.GuideWorld))
            {
                Dnd.SoundAdapter.PlayMusic("guideBattle3",1);
            } else
            {
                if( this.battleType == 2 )  //Pvp
                {
                    Dnd.SoundAdapter.PlayMusic("battlePvP", 1);
                }
                // if (message.dId > 2000) //精英.
                // {
                //     Dnd.SoundAdapter.PlayMusic("battle2", 1);
                // } else  //普通副本.
                else
                {
                    Dnd.SoundAdapter.PlayMusic("battle1", 1);
                }  
            }             
        }
        ...
        p.InitUI = function()
        {
            ...
            var show_box = (this.battleManager.message.battleType != Dnd.BattleType.Pvp);
            this.battlePanel = Dnd.UIUtils.addBattlePanel(this.fgUI, 0, battle_panel_y, 400, show_box);              
            ...
            var game_w = Dnd.Env.gameWidth;
            var game_h = Dnd.Env.gameHeight;
            var btn_w = 124;
            ...
            this.battleManager.listMyHero.ForEach(this, function(actor, index)
            {
                ...
                if (Dnd.Device.isIphoneX&&window.ua.indexOf('micromessenger') == -1)
                {
            ...
            Dnd.UIUtils.scale(btn_stopVideo);
            ...
            Dnd.UIUtils.scale(btn_pause);
            btn_pause.on(Event.CLICK, this, this.PauseDialog);
            if (!Dnd.GuideManager.isGuide 
            ...
            // if( !window.isBTversion )
            // {
            //     var btn_fast_x = game_w - 15;
            //     // var btn_fast_y = game_h - SUPER_BTN_H - 15;
                

            //     var btn_fast_y = 115 + window.bangSize;
            //     if (window.HANDLE_MINIGAME_CLOSE)
            //     {
            //         btn_fast_y += 80;
            //     }
            //     var btn_fast = new Image("total_"+ window.language +"/img_siege_exit.png");            
            //     btn_fast.anchorX = 1;
            //     btn_fast.anchorY = 0;
            //     if(window.testScreen)
            //     {
            //         btn_fast_y = game_h - btn_fast.height - 50;
            //     }
            //     btn_fast.pos(btn_fast_x, btn_fast_y);
            //     Dnd.UIUtils.scale(btn_fast);
            //     btn_fast.on(Event.CLICK, this, this._FastFinishBattle);
            //     if ((this.battleType == Dnd.BattleType.Dungeon)
            //         && (!Dnd.GuideManager.isGuide)
            //         && (this.battleManager.message.dId != 1000) 
            //         && (this.battleManager.message.dId != 1006)
            //         && (this.battleManager.message.dId != 1004)
            //         && (this.battleManager.message.dId != 1013))
            //     {
            //         this.fgUI.addChild(btn_fast);  //副本战斗，无剧情，才显示.
            //         btn_fast.existence = true;
            //     }    
            //     this._btnFastFinish = btn_fast; 
            //     this.SetBtnFastFinishEnable(true);
            // }
 

            // var tick_bg = new Image("total/img_cityTitle.png");
            // tick_bg.anchorX = 0.5;
            // tick_bg.anchorY = 0;
            // tick_bg.x = 98 * 0.5;
            // tick_bg.y = 0 - 26;
            // if (!this._unlimitSkip)    //没有开启无限制跳过.
            // {
            //     this._btnFastFinish.addChild(tick_bg);
            // }
            // this._fastTickBg = tick_bg;
            
            // var tick_label = new Label(GameText[40209] + ":" + this._freeSkipLeft);
            // tick_label.fontSize = 24;
            // tick_label.align = "center";
            // tick_label.color = "#FFFFFF";
            // tick_label.anchorX = 0.5;
            // tick_label.anchorY = 0.5;
            // tick_label.x = tick_bg.width / 2;
            // tick_label.y = tick_bg.height / 2;
            // tick_bg.addChild(tick_label);
            // this._fastTickLabel = tick_label;

            ///this.SetBtnFastFinishEnable(false);
            ...
            if ( this._hasSpeedUpFeature) //加速功能
            {
                ...
                Dnd.UIUtils.scale(btn_speed_on);
                ...
                Dnd.UIUtils.scale(btn_speed_off);
                ...
                if( Dnd.Player.speedUpCanUse ) //达成条件
                {
                    if( Dnd.Player.speedUpSelected )
            ...
            if ( this._hasAutoSuperFeature) //自动战斗
            {
                ...
                Dnd.UIUtils.scale(btn_auto_off);
                ...
                if( Dnd.Player.autoFightCanUse ) //达成条件
                {
                    if( Dnd.Player.autoFightSelected )
            ...
            if (this.battleType == Dnd.BattleType.Hunt)
            {
            ...
            Dnd.UIUtils.scale(img_boss_warning);
        ...
        p._OnResize = function()
        {
            ...
            {
                this._nodeSuperButton.y = Dnd.Env.gameHeight;
                if (Dnd.Device.isIphoneX&&window.ua.indexOf('micromessenger') == -1)
                {
                    this._nodeSuperButton.y -= 25;
                } 
        ...
        {
            this.curState = Dnd.SceneState.Normal;
            ...
            {
                if( this.battleType == Dnd.BattleType.Pvp )  //Pvp
                {
                    Dnd.SoundAdapter.PlayMusic("battlePvP", 1);
                } else
                {
        ...
        {
            var show_box = (this.battleManager.message.battleType != Dnd.BattleType.Pvp);
            this.battlePanel = Dnd.UIUtils.addBattlePanel(this.fgUI, 0, 0, 400, show_box);  
            ...
            // if (!window.isBTversion && this.battleType == Dnd.BattleType.Dungeon)
            // {
            //     var btn_fast_y = 115;
            ...
            //     btn_fast.anchorX = 1;
            //     btn_fast.anchorY = 0;
            //     btn_fast.pos(game_w - 15, btn_fast_y);
            //     Dnd.UIUtils.scale(btn_fast);
            //     btn_fast.on(Event.CLICK, this, this._FastFinishBattle);
            //     this.fgUI.addChild(btn_fast);  //屏蔽.
            //     this._btnFastFinish = btn_fast;                                
            // }
        ...
        {
            var head = Dnd.UIUtils.addBattleHeroHead(this._nodeSuperButton, x, y, actor.tid, actor.quality);                   
            head.actor = actor;
            ...
            head.OnMpChanged = function(rate)
            {
                ...
                {
                    Dnd.SoundAdapter.PlaySound("SuperReady");
                    if (this.actor.isMyGroup && this.actor.battleManager.scene.battleType == Dnd.BattleType.Dungeon)                         
                    {
                        // console.log("SuperReady");
                        if (this.actor.battleManager.message.dId == 1001   //开关.
                            && !Dnd.GuideManager.checkPhaseGuide(Dnd.GuideStep.ReleaseSkill)    //没有触发过ReleaseSkill.
                            && !Dnd.GuideManager.isGuiding(Dnd.GuideStep.ReleaseSkill)          //没有正在执行ReleaseSkill.
                            && !Dnd.GuideManager.isGuiding(Dnd.GuideStep.ReleaseSkill_2)        //没有正在执行ReleaseSkill_2.
                            && !Dnd.GuideManager.isGuiding(Dnd.GuideStep.GuideWorld)            //没有正在执行GuideWorld.
                            && !Dnd.GuideManager.isGuiding(Dnd.GuideStep.SuoErJoinFight)       //没有正在执行SuoErJoinFight.
                            && !Dnd.GuideManager.isGuiding(Dnd.GuideStep.SuoErJoinFight_2)     //没有正在执行SuoErJoinFight_2.
                            )
                        {
                            Dnd.GuideManager.startPhaseGuide(Dnd.GuideStep.ReleaseSkill, 1);
                            Dnd.GuideManager.nextStep(Dnd.GuideStep.ReleaseSkill, Dnd.Guide_ReleaseSkill.Click_hero, true, null, this);  
                        }
                    }
                }
            ...
            {                
                var eff_url = Dnd.Porting.resPath + "/UIEffect/E_UI_jinengBtn.lh";
                // this.actor.battleManager.scene._arrDeepClearUrl.push(eff_url);
                var scale = Dnd.Env.gameWidth / (Dnd.Env.gameHeight / 16 * 9);
                if(window.isPC){
                    scale=1;
                }
                Dnd.UIUtils.showUIParticle(this.m_effect, eff_url, scale);         
            };
            ...
            head._DeleteEffect = function()
            {
                ...
                //删除特效
                Dnd.UIUtils.removeUIParticle(this.m_effect);
            };
        ...
        {
            if (this.battleType == Dnd.BattleType.Pvp)
            {
                Dnd.UIUtils.prompt(GameText[90000]);
                return;
            }
            ...
            {
                if(!first)
                    Dnd.UIUtils.prompt("VIP6或领主30级可使用自动战斗");
                this.battleManager.SetAutoSuper(false);
                return;
            }
            else
            {
                Dnd.Player.autoFightSelected = is_on;
            } 
        ...
        p._SetSpeedUp = function(is_on, first)
        {
            ...
            {
                if(!first)
                    Dnd.UIUtils.prompt("VIP7或领主40级可使用快速战斗");
                this.battleManager.SetSpeedUp(false);
                return;
            }
            else
            {
                Dnd.Player.speedUpSelected = is_on;
            } 
        ...
        p._FastFinishBattle = function()
        {
            if (this.battleType == Dnd.BattleType.Dungeon)
            {
                ...
                {                    
                    Dnd.UIUtils.prompt(this._freeSkipText);
                }
            }
        };
        ...
        p.startVideo = function()
        {
            ...
            //     var timeBol = true;
            //     _timer.once(3000, this, function(timeBol)
            //     {
            //         timeBol = false;                
            //     },[timeBol]);
            // }
            ...
            //     if(timeBol)
            //     {
            //         Dnd.UIUtils.prompt("录屏失败：录屏时长低于 3 秒");
            //     }
            //     else
            //     {
        ...
        {            
            ...
            {
                case Dnd.BattleType.Dungeon:
                    type = 24;
                    break;

                case Dnd.BattleType.Pvp:
                ...
                case Dnd.BattleType.Hunt:
                    type = 24;
                    break;
            }
            this.pauseDialog = Dnd.UIUtils.warnWindowById(type, this,[], "_ExitPause", "_ResumePause");
        ...
        p._ExitPause = function()
        {
            ...
            if (this.battleType == Dnd.BattleType.Pvp && !this.battleManager.isReplayMode)
            {
            ...
            //放弃战斗时.
            if (Dnd.GuideManager.isGuiding(Dnd.GuideStep.ReleaseSkill))   //结束引导.
            {
                Dnd.GuideManager.nextStep(Dnd.GuideStep.ReleaseSkill, Dnd.Guide_ReleaseSkill.Click_hero, false);  
            }             
            ...
            {
                this.battleManager.GiveUpBattle();
                if( Dnd.Player.heroEquipmentSaoDangFight ) //从英雄装备进扫荡界面进挑战
                {                    
                    var req = new QUERY_DUNGEON_REQ();                
                    req.dId = this.battleManager.message.dId;
                    req.from = 0;
                    Dnd.Network.send(req);  
                                                        
                }
                else
                {
                    var fuben_data = Dnd.FubenData.GetFubenData(this.battleManager.message.dId);                  
            ...
            {
                Dnd.UIManager.showFunction(true); 
                switch (this.battleType)
                {
                    case Dnd.BattleType.Pvp:                        
            ...
            Dnd.Mainmenu.firstToCity = true;
            Dnd.Mainmenu.openCityFromBattle(handler);                               
        };
        ...
        p._SuperSkill = function(actor)
        {
            ...
            Dnd.Player.clientOperationLog('副本战斗放大招' + actor.name); 
        ...
        p.OnFrameUpdate = function(frame_tick)
        {
            if (!this.battleManager)	//胡闹补丁.
			{
				...
				Dnd.Mainmenu.loadingToCity(null);
			}
        ...
        p._UpdateUI = function(is_force)
        {
            ...
            {
                // var date = new Date(wave_status.tick);
                // var min = date.getMinutes();
                // var sec = date.getSeconds();
                // var str_min = min > 9 ? min : "0" + min;
                // var str_sec = sec > 9 ? sec : "0" + sec;
                // this.battlePanel.m_time.text = str_min + ":" + str_sec;

               this.battlePanel.m_time.text = Dnd.UIUtils.formatTime(wave_status.tick * 0.001, false);  
            }
            ...
            // var fast_finish_tick_status = this.battleManager.GetFastFinishTickStatus();
            // var is_fast_finish_ticking = fast_finish_tick_status.isTicking;
            // if (is_fast_finish_ticking || is_force)
            // {
            //     this._fastTickLabel.text = Math.floor(fast_finish_tick_status.tick * 0.001);
            // }
        };
...
// var num_before = ResourceManager.currentResourceManager.getResourcesLength();
// console.log(">>释放前:" + num_before);
            if (this._dragon)
            {
                this._dragon.Clean();
                this._dragon = null;
            ...
            if (Dnd.Porting.effConfig.keepBattleRes && this.battleType == Dnd.BattleType.Dungeon)
            {
                var arr_cache_res_url = Dnd.Player.fubenResCache.GetCacheResList();                
                this._arrDeepClearUrl = Array.Exclude(this._arrDeepClearUrl, arr_cache_res_url);
            }
            this.Clean3DBase(true);
            
            Dnd.Particle3D.ClearPool(Dnd.Porting.resPath + "/UIEffect/E_UI_jinengBtn.lh");
            Dnd.Particle3D.ClearPool(Dnd.Porting.resPath + "/UIEffect/E_UI_zhandoushengli.lh");
            Dnd.Particle3D.ClearPool(Dnd.Porting.resPath + "/UIEffect/E_UI_star.lh");
            Dnd.Particle3D.ClearPool(Dnd.Porting.resPath + "/UIEffect/E_UI_star_1.lh");
            Dnd.Particle3D.ClearPool(Dnd.Porting.resPath + "/UIEffect/E_UI_star_2.lh");

            // if (!Dnd.Porting.effConfig.keepBattleRes)
            // {
            //     _loader.DeepClearResDir(Dnd.Porting.resPath + "/Hero/");                
            //     _loader.DeepClearResDir(Dnd.Porting.resPath + "/Monster/");
            //     _loader.DeepClearResDir(Dnd.Porting.resPath + "/Battle/Effect/");
            // }
            // _loader.DeepClearResDir(Dnd.Porting.resPath + "/Battle/Treasure.zip");

// var num_after = ResourceManager.currentResourceManager.getResourcesLength();            
// console.log(">>释放后:" + num_after);   
// console.log(">>释放了:" + (num_before - num_after));           
